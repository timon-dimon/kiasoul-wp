...
function catchbox_setup() {
	...
	add_image_size( 'featured-slider', 196, 89, true ); // Used for featured posts if a large-feature doesn't exist
...
add_filter( 'excerpt_length', 'catchbox_excerpt_length', 9 );
...
function catchbox_continue_reading_link() {
	return ' <a class="more-link" href="'. esc_url( get_permalink() ) . '">' . __( 'Читать далее', 'catchbox' ) . '</a>';
}
...
function catchbox_posted_on() {
	printf( __( '<time class="entry-date updated" datetime="%3$s" pubdate>%4$s</time>', 'catchbox' ),
